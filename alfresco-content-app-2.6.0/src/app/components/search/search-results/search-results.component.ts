...
export class SearchResultsComponent extends PageComponent implements OnInit {
  ...
  onNodeDoubleClick(node: MinimalNodeEntity) {
    if (node && node.entry) {
      if (node.entry.isFolder) {
       // if(node.entry.nodeType == 'edms:bppbuildingfolder')
       if(node.entry.nodeType == 'cm:folder')
        this.router.navigateByUrl('/edms/building-info;id=' + encodeURIComponent(node.entry.id));
        else{
  ...
  launchHbdrClassicView(){
    window.open(
      this.ecmHost+"/share/page/site/hbdr/dp/ws/faceted-search#searchTerm="+this.searchedWord+"&scope=hbdr", "_blank");
  }
