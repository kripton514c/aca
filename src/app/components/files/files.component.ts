...
export class FilesComponent extends PageComponent implements OnInit, OnDestroy {
  ...
  routeToReports() {
    this.router.navigateByUrl(`/search/sites?node=${this.node.id}`);
  };
      ...
      this.contentApi.getNode(nodeId).subscribe(
        (node) => {
          ...
          if (node.entry && node.entry.isFolder) {
            ...
            this.checkFolderUnderSite(node.entry);
          } else {
  ...
  checkFolderUnderSite(node: MinimalNodeEntryEntity) {
    let site: any;
    if (node.path.elements)
      site = node.path.elements.find((s) => s.nodeType === 'st:site');
    if (site)
